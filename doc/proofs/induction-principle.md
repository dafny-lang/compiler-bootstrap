...
HO note explores a way of factorizing proofs when writing a collection of lemmas about a
...
which we will use as a motivating example.
...
Such a lemma has to be proved by induction over `e` for all the transformations we wish to
...
**Note:** if the AST contains loops, the proofs can't be performed *only* by induction on the
syntax. If we use a notion of fuel, we need to do the induction also on the fuel. Let's assume
we don't have loops for now.
...
```dafny
predicate method IsPure_WithLocals(locals: set<string>, e: Expr)
  decreases e
  // `locals`: the set of local variables which have been introduced by outer
  // let-bindings (if the sub-expression `e` only updates those variables then
  // the ancestor expression is pure, because those variables don't escape its scope).
{
  match e
    case Var(name) => true
    case Literal(n) => true
    case Bind(bvar, bval, body) =>
      && IsPure_WithLocals(locals, bval)
      && IsPure_WithLocals({bvar} + locals, body)
    case Assign(avar, aval) =>
      avar in locals && IsPure_WithLocals(locals, aval)
    case If(cond, thn, els) =>
      && IsPure_WithLocals(locals, cond)
      && IsPure_WithLocals(locals, thn)
      && IsPure_WithLocals(locals, els)
    case Op(op, oe1, oe2) =>
      IsPure_WithLocals(locals, oe1) && IsPure_WithLocals(locals, oe2)
    case Seq(e1, e2) =>
      IsPure_WithLocals(locals, e1) && IsPure_WithLocals(locals, e2)
}

predicate method IsPure(e: Expr)
{
  IsPure_WithLocals({}, e)
}
```
...
{
  // The precondition of ``InterpExpr_Pure_WithLocals``:
  IsPure_WithLocals(locals, e) ==>
  ctx.Keys >= locals ==>
  // The postcondition of ``InterpExpr_Pure_WithLocals``:
  match InterpExpr(e, ctx)
    case Success((_, ctx')) =>
      && ctx'.Keys == ctx.Keys
      && forall x | x in ctx.Keys && x !in locals :: ctx'[x] == ctx[x]
...
```dafny
lemma InterpExpr_Pure_WithLocals_Seq(e1: Expr, e2: Expr)
...
```dafny
lemma InterpExpr_Pure_WithLocals(e: Expr)
  ensures forall locals, ctx :: P(locals, ctx, e)
{
  match e
    case Seq(e1, e2) =>
      // Induction hypothesis for `e1`
      InterpExpr_Pure_WithLocals(e1);

      // Induction hypothesis for `e2`
      InterpExpr_Pure_WithLocals(e2);

      // Proof of the induction step for `Seq`
      InterpExpr_Pure_WithLocals_Seq(e1, e2);
