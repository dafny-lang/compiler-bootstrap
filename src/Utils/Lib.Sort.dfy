...
module Utils.Lib.ArraySort {
  ...
  trait Sorter<T> {
    ...
    twostate lemma SetOfSliceShuffled(arr: array<T>, lo: int, hi: int)
      ...
      ensures Set.OfSlice(arr, lo, hi) == old(Set.OfSlice(arr, lo, hi))
    {
      calc {
        old(Set.OfSlice(arr, lo, hi));
        set x <- old(arr[lo..hi]);
        set x <- old(multiset(arr[lo..hi]));
        set x <- multiset(arr[lo..hi]);
        set x <- arr[lo..hi];
        Set.OfSlice(arr, lo, hi);
      }
    }
    ...
    twostate lemma SortableShuffled(arr: array<T>, lo: int, hi: int)
      requires 0 <= lo <= hi <= arr.Length
    ...
    twostate lemma SameElementsShuffled(arr: array<T>, lo: int, hi: int)
      requires 0 <= lo <= hi <= arr.Length
      requires old(Sortable(arr, lo, hi))
      requires SameElements(arr, lo, hi)
      ensures Shuffled(arr, lo, hi)
    ...
    lemma SwapEq(
      ...
      requires 0 <= lo <= left <= mid < right <= hi <= arr.Length
      requires C.Striped(arr[..], pivot, lo, left, mid, right, hi)
      requires Cmp(arr[mid], pivot).Eq?
      ensures SameElements(arr, lo, hi)
      ensures C.Striped(arr[..], pivot, lo, left, mid + 1, right, hi)
    {
      reveal C.Striped();
    }
  ...
  class PredicateSorter<T> extends Sorter<T> {
    const cmp: (T, T) -> Cmp

    constructor(cmp: (T, T) -> Cmp) ensures this.cmp == cmp {
      this.cmp := cmp;
    }
...
module Utils.Lib.SetSort {
    ...
    ensures cmp.Sorted(sq)
  {
    ...
    assert Array.Set.OfArray(arr) == st;
    var sorter := new ArraySort.PredicateSorter(cmp.cmp);
    label unsorted:
